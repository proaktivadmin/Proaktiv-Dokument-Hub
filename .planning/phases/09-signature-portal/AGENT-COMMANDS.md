# Signature Portal - Agent Commands

Copy and paste each command block into a new agent conversation. Execute in order.
...
```typescript
export function useSignature(employeeId: string, version: 'with-photo' | 'no-photo') {
  // Fetch from GET /api/signatures/{employeeId}?version={version}
  // Return: { signature, isLoading, error, refetch }
}

export function useSendSignature(employeeId: string) {
  // POST to /api/signatures/{employeeId}/send
...
Return all 6 handover summaries to the orchestrator agent for:
1. Code review
2. Lint check
3. Integration test
