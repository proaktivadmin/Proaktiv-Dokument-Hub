...
async def get_inventory_stats(
    ...
    return InventoryStatsResponse(
        stats=InventorySyncStats(**sync_stats), missing_templates=[MissingTemplateInfo(**t) for t in missing_templates]
    )


@router.get("/inventory/by-phase", response_model=dict[str, dict[str, int]])
async def get_inventory_by_phase(db: AsyncSession = Depends(get_db)):
    """
