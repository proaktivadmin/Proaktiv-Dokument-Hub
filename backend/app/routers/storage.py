...
from app.services.webdav_service import StorageItem, get_webdav_service
...
async def get_storage_status():
    ...
    try:
        ...
        return StatusResponse(configured=True, connected=False, message=f"Connection error: {str(e)}")


@router.get("/debug")
async def debug_storage(
    path: str = Query("/", description="Directory path to test"), user: dict = Depends(get_current_user)
):
    """
  ...
  <prop>
    ...
    <getlastmodified/>
            ...
            async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
                response = await client.request("PROPFIND", url, headers=headers, content=propfind_body)

                # Also try without trailing slash
                url_no_slash = settings.WEBDAV_URL.rstrip("/")
                response2 = await client.request("PROPFIND", url_no_slash, headers=headers, content=propfind_body)
...
async def browse_storage(
    path: str = Query("/", description="Directory path to browse"), user: dict = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        return BrowseResponse(
            path=path, items=[_item_to_response(item) for item in items], parent_path=_get_parent_path(path)
        )
        ...
        if "401" in error_msg:
            raise HTTPException(
                status_code=401, detail="WebDAV authentication failed. Check WEBDAV_USERNAME and WEBDAV_PASSWORD."
            )
        elif "404" in error_msg:
            raise HTTPException(status_code=404, detail=f"Directory not found: {path}")
        elif "connection" in error_msg.lower() or "refused" in error_msg.lower():
            raise HTTPException(status_code=503, detail="Cannot connect to WebDAV server. Check WEBDAV_URL.")
        else:
...
async def download_file(
    path: str = Query(..., description="File path to download"), user: dict = Depends(get_current_user)
):
    """
...
async def upload_file(
    ...
    try:
        ...
        return {"success": True, "path": dest_path, "filename": file.filename, "size": len(content)}
    except RuntimeError as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_directory(
    path: str = Form(..., description="Directory path to create"), user: dict = Depends(get_current_user)
):
    """
...
@router.post("/move")
async def move_file(request: MoveRequest, user: dict = Depends(get_current_user)):
    """
...
async def delete_file(path: str = Query(..., description="Path to delete"), user: dict = Depends(get_current_user)):
    """
...
async def import_to_library(
    request: ImportRequest, db: AsyncSession = Depends(get_db), user: dict = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        await AuditService.log(
            ...
            details={"title": request.title, "source_path": request.path, "file_name": info.name},
        )
