...
class TemplateSettingsService:
    ...
    async def update_settings(
        ...
        *,
        ...
        if header_template_id:
            await TemplateSettingsService.validate_layout_partial(
                db, header_template_id, 'header'
            )
            template.header_template_id = header_template_id
        
        if footer_template_id:
            await TemplateSettingsService.validate_layout_partial(
                db, footer_template_id, 'footer'
            )
    ...
    async def validate_layout_partial(
        ...
        if not partial:
            raise HTTPException(
                status_code=400,
                detail=f"Layout partial {partial_id} not found"
            )
