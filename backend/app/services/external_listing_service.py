...
class ExternalListingService:
    ...
    async def list(
        ...
        *,
        ...
        query = select(ExternalListing).options(
            selectinload(ExternalListing.office), selectinload(ExternalListing.employee)
        )
    ...
    @staticmethod
    async def get_by_id(db: AsyncSession, listing_id: UUID) -> ExternalListing | None:
        """Get a listing by ID."""
        result = await db.execute(
            select(ExternalListing)
            .options(selectinload(ExternalListing.office), selectinload(ExternalListing.employee))
    ...
    @staticmethod
    async def update(db: AsyncSession, listing_id: UUID, data: ExternalListingUpdate) -> ExternalListing | None:
    ...
    @staticmethod
    async def verify(
        db: AsyncSession, listing_id: UUID, *, status: str = "verified", verified_by: str, notes: str | None = None
    ...
    async def get_by_office(
        db: AsyncSession, office_id: UUID, *, status: str | None = None
    ) -> builtins.list[ExternalListing]:
        """Get all listings for an office."""
        query = select(ExternalListing).where(ExternalListing.office_id == str(office_id))
    ...
    async def get_by_employee(
        ...
        query = select(ExternalListing).where(ExternalListing.employee_id == str(employee_id))
    ...
    @staticmethod
    async def get_needing_attention(db: AsyncSession) -> builtins.list[ExternalListing]:
