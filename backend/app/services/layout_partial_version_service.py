...
class LayoutPartialVersionService:
    ...
    @staticmethod
    async def list_versions(db: AsyncSession, partial_id: UUID) -> tuple[list[LayoutPartialVersion], int]:
        """
        ...
        partial_result = await db.execute(select(LayoutPartial).where(LayoutPartial.id == str(partial_id)))
    ...
    @staticmethod
    async def get_version(db: AsyncSession, partial_id: UUID, version_number: int) -> LayoutPartialVersion | None:
    ...
    async def create_version(
        db: AsyncSession, partial_id: UUID, data: LayoutPartialVersionCreate
        ...
        result = await db.execute(
            select(func.max(LayoutPartialVersion.version_number)).where(
                LayoutPartialVersion.partial_id == str(partial_id)
            )
        )
    ...
    @staticmethod
    async def revert_to_version(db: AsyncSession, partial_id: UUID, version_number: int, *, reverted_by: str) -> dict:
        """
        ...
        version = await LayoutPartialVersionService.get_version(db, partial_id, version_number)
        if not version:
...
class LayoutPartialDefaultService:
    ...
    async def list(
        ...
        *,
        ...
        query = select(LayoutPartialDefault).options(
            selectinload(LayoutPartialDefault.partial), selectinload(LayoutPartialDefault.category)
        )
    ...
    @staticmethod
    async def get_by_id(db: AsyncSession, default_id: UUID) -> LayoutPartialDefault | None:
        """Get a default rule by ID."""
        result = await db.execute(
            select(LayoutPartialDefault)
            .options(selectinload(LayoutPartialDefault.partial), selectinload(LayoutPartialDefault.category))
    ...
    @staticmethod
    async def update(
        db: AsyncSession, default_id: UUID, data: LayoutPartialDefaultUpdate
    ) -> LayoutPartialDefault | None:
    ...
    async def get_default_for_context(
        ...
        *,
        ...
        all_result = await db.execute(
            query.where(LayoutPartialDefault.scope == "all").order_by(LayoutPartialDefault.priority.desc()).limit(1)
        )
