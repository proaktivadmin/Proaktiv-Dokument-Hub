...
class LayoutPartialService:
    ...
    async def get_by_id(db: AsyncSession, partial_id: UUID) -> Optional[LayoutPartial]:
        ...
        result = await db.execute(
            select(LayoutPartial).where(LayoutPartial.id == partial_id)
        )
    ...
    async def get_default(
        ...
        *,
        ...
        result = await db.execute(
            ...
            .where(LayoutPartial.is_default == True)
        )
        return result.scalar_one_or_none()
    ...
    async def delete(db: AsyncSession, partial: LayoutPartial) -> None:
        ...
        header_count_result = await db.execute(
            ...
            .where(Template.header_template_id == partial.id)
        )
        footer_count_result = await db.execute(
            select(func.count(Template.id))
            .where(Template.footer_template_id == partial.id)
        )
        ...
        if header_count > 0 or footer_count > 0:
            raise HTTPException(
                status_code=409,
                detail=f"Partial is in use by {header_count + footer_count} template(s)"
            )
