...
class SanitizerService:
    ...
    OLD_STYLES_PATTERN = re.compile(r'(font-family|font-size|color|background-color)\s*:[^;"]+;?', re.IGNORECASE)
    ...
    def _is_already_valid(self, soup: BeautifulSoup) -> tuple[bool, str]:
        ...
        for marker in self.SYSTEM_MARKERS:
            current_marker = marker  # Bind to local variable for lambda
            if soup.find(class_=lambda x, m=current_marker: x and m in str(x).lower()):
    ...
    def _ensure_stilark_reference(self, wrapper: Tag) -> None:
        ...
        stilark_span = BeautifulSoup(
            f'<span vitec-template="{self.VITEC_STILARK_RESOURCE}">&nbsp;</span>', "html.parser"
        ).find("span")
...
_sanitizer_service: SanitizerService | None = None
