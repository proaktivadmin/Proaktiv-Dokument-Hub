...
class PostalCodeService:
    ...
    async def list(db: AsyncSession) -> List[PostalCode]:
        ...
        result = await db.execute(
            select(PostalCode).order_by(PostalCode.postal_code)
        )
        return list(result.scalars().all())
    
    @staticmethod
    async def get_by_code(db: AsyncSession, postal_code: str) -> Optional[PostalCode]:
            ...
            async with httpx.AsyncClient() as client:
                ...
                for line in lines:
                    ...
                    if len(parts) >= 5:
                        ...
                        stmt = pg_insert(PostalCode).values(
...
class OfficeTerritoryService:
    ...
    async def list(
        ...
        *,
        ...
        query = select(OfficeTerritory).options(
            selectinload(OfficeTerritory.office),
            selectinload(OfficeTerritory.postal_code_info)
        )
    ...
    @staticmethod
    async def get_by_id(db: AsyncSession, territory_id: UUID) -> Optional[OfficeTerritory]:
    ...
    @staticmethod
    async def get_blacklist(db: AsyncSession) -> List[str]:
        """Get all blacklisted postal codes."""
        result = await db.execute(
            select(OfficeTerritory.postal_code)
    ...
    @staticmethod
    async def get_available_sources(db: AsyncSession) -> List[str]:
        """Get list of sources that have territories."""
        result = await db.execute(
            select(OfficeTerritory.source).distinct()
        )
        return [row[0] for row in result.all()]
    ...
    async def import_from_csv(
        ...
        for row in data:
            try:
                ...
                # Upsert territory
                stmt = pg_insert(OfficeTerritory).values(
                    office_id=str(office_id),
                    postal_code=postal_code,
                    source=source,
