...
class CodePatternService:
    ...
    async def get_list(
        ...
        *,
        ...
        if search:
            search_filter = or_(
                CodePattern.name.ilike(f"%{search}%"),
                CodePattern.description.ilike(f"%{search}%")
            )
    ...
    async def get_by_id(db: AsyncSession, pattern_id: UUID) -> Optional[CodePattern]:
        ...
        result = await db.execute(
            select(CodePattern).where(CodePattern.id == pattern_id)
        )
    ...
    async def get_categories(db: AsyncSession) -> List[str]:
        ...
        """
        result = await db.execute(
            select(CodePattern.category)
            .distinct()
            .order_by(CodePattern.category)
        )
