...
class InventoryService:
    ...
    async def get_sync_stats(db: AsyncSession) -> Dict[str, Any]:
        ...
        total_vitec = await db.scalar(
            select(func.count(VitecTemplateRegistry.id))
        ) or 0
        
        # Get total local templates
        total_local = await db.scalar(
    ...
    async def get_missing_templates(db: AsyncSession, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        query = select(VitecTemplateRegistry).where(
            VitecTemplateRegistry.sync_status == 'missing'
        ).limit(limit)
    ...
    async def update_sync_status(
        ...
        query = select(VitecTemplateRegistry).where(
            VitecTemplateRegistry.id == uuid_lib.UUID(registry_id)
        )
        result = await db.execute(query)
