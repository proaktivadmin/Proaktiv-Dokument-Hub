...
class MergeFieldService:
    ...
    async def get_by_id(db: AsyncSession, field_id: UUID) -> Optional[MergeField]:
        ...
        result = await db.execute(
            select(MergeField).where(MergeField.id == field_id)
        )
    ...
    async def get_by_path(db: AsyncSession, path: str) -> Optional[MergeField]:
        ...
        """
        result = await db.execute(
            select(MergeField).where(MergeField.path == path)
        )
        return result.scalar_one_or_none()
    ...
    async def get_categories(db: AsyncSession) -> List[str]:
        ...
        """
        result = await db.execute(
            select(MergeField.category)
            .distinct()
            .order_by(MergeField.category)
        )
    ...
    async def search_autocomplete(
        ...
        """
        search_filter = or_(
            MergeField.path.ilike(f"%{query}%"),
            MergeField.label.ilike(f"%{query}%")
        )
        
        result = await db.execute(
            select(MergeField)
            .where(search_filter)
            .order_by(MergeField.usage_count.desc())
            .limit(limit)
        )
    ...
    async def discover_all(
        ...
        *,
        ...
        result = await db.execute(
            select(Template)
            .where(Template.file_type == "html")
            .where(Template.content.isnot(None))
        )
