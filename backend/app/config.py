...
class Settings(BaseSettings):
    ...
    def validate_secret_key(cls, v: str, info) -> str:
        ...
        if len(v) < 32:
            warnings.warn(
                f"SECRET_KEY is only {len(v)} characters. Recommended: 32+ characters for security.",
                UserWarning,
                stacklevel=2,
            )
...
def get_mock_user() -> dict:
    ...
    """
    return {"email": settings.MOCK_USER_EMAIL, "name": settings.MOCK_USER_NAME, "is_admin": True}
