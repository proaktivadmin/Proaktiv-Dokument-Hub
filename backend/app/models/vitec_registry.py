...
class VitecTemplateRegistry(Base):
    ...
    vitec_name: Mapped[str] = mapped_column(
        String(255), nullable=False, unique=True, comment="Official Vitec template name"
    )
    vitec_type: Mapped[str] = mapped_column(String(50), nullable=False, comment="Template type: Objekt/Kontakt, System")
    vitec_phase: Mapped[str | None] = mapped_column(
        String(50), nullable=True, comment="Primary phase: Innsalg, Til salgs, Klargjoring, etc."
    )
    vitec_category: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="Vitec document category")
    vitec_channel: Mapped[str | None] = mapped_column(String(20), nullable=True, comment="Channel: pdf, email, sms")

    # Description from Vitec
    description: Mapped[str | None] = mapped_column(Text, nullable=True, comment="Description of the template purpose")

    # Sync with local templates
    local_template_id: Mapped[uuid.UUID | None] = mapped_column(
        GUID, ForeignKey("templates.id", ondelete="SET NULL"), nullable=True, comment="FK to local template if synced"
    )

    sync_status: Mapped[str] = mapped_column(
        String(20), default="missing", comment="Status: synced, missing, modified, local_only"
    )

    # Tracking
    last_checked: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="Last time sync status was checked"
    )
    ...
    # Relationships
    local_template = relationship("Template", foreign_keys=[local_template_id], backref="vitec_registry_entry")
