...
class LibraryImporter:
    ...
    def __init__(self, source_path: Path, dry_run: bool = False, skip_azure: bool = False):
    ...
    async def run(self):
        ...
        async with async_session_factory() as session:
            ...
            categories_needed = {cat for _, cat in all_files if cat}
            ...
            for idx, (file_path, category_name) in enumerate(all_files, 1):
                ...
                await self._import_template(
                    session, file_path=file_path, category_name=category_name, progress_info=progress_info
                )
    ...
    def _collect_files(
        self, directory: Path, parent_category_name: str | None = None
        ...
        if category:
            ...
            try:
                category = await CategoryService.create(
                    session, name=name, icon=icon, description=f"Imported from legacy library: {name}"
                )
    ...
    async def _import_template(
        self, session, file_path: Path, category_name: str | None = None, progress_info: str | None = None
    ):
        """
        ...
        if not self.skip_azure and self.storage_service.is_configured:
            ...
            blob_url = await self.storage_service.upload_file(
                file_data=io.BytesIO(file_bytes), blob_name=blob_name, content_type=content_type
            )
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Import templates from library")
    parser.add_argument(
        "--source",
        type=str,
    ...
    parser.add_argument("--skip-azure", action="store_true", help="Skip Azure upload (store content in DB only)")
    ...
    # Run the import
    importer = LibraryImporter(source_path=source_path, dry_run=args.dry_run, skip_azure=args.skip_azure)
